...
class EdinPlusLightChannelInstance:
    ...
    async def set_brightness(self, intensity: int):
        await async_send_to_npu(self._endpoint,f"$ChanFade,1,12,{self._channel},{str(intensity)},0;")
        self._brightness = intensity

    async def turn_on(self):
        await async_send_to_npu(self._endpoint,f"$ChanFade,1,12,{self._channel},255,0;")
        self._is_on = True

    async def turn_off(self):
        await async_send_to_npu(self._endpoint,f"$ChanFade,1,12,{self._channel},0,0;")
        self._is_on = False
